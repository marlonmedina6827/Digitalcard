...
export function AddEditTableForm(props) {
  ...
  return (
    <Form className='add-edit-table-form' onSubmit={formik.handleSubmit}>
        <Form.Input name="number" type='number' placeholder="Numero e la mesa" value={formik.values.number} onChange={formik.handleChange} error={formik.errors.number} />

        <Button type='submit' primary fuild content={table ? "Actualizar" : "Crear"} />
    </Form>
  )
}
